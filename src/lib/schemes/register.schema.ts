...
export const registerSchema = z
  .object({
    ...
    email: z.string({ required_error: "Email is required" }).email("Email is invalid").min(1, "Email is required"),
    password: z
      .string({ required_error: "Password is required" })
      .regex(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^A-Za-z\d])[A-Za-z\d\S]{8,100}$/)
      .min(1, "Password is required"),
    rePassword: z.string({ required_error: "Confirm password is required" }).min(1, "RePassword is required"),
    phone: z
