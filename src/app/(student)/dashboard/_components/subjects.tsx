...
import CardSkeleton from "@/components/customs/card-skeleton";
import AlertError from "@/components/customs/alert-error";

export default function FetchSubjects({ token }: { token: string }) {
  const { data, error, isPending } = useSubjects(token);
      ...
      {data &&
        data.subjects.map((subject: Subject) => {
          return (
            <>
              <div
                className="w-[330px] h-[292px] relative group cursor-pointer"
                key={subject._id}
              >
